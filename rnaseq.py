...
parser.add_argument('-s', action='store_true', help="If selected, script will be generated but not submitted to slurm")
...
if  os.path.isfile(args.bamIn):
	...
	with open(job_file,"w") as fh:
		...
		fh.writelines("\n")
		fh.writelines("bamtofastq \\\n")
                fh.writelines("F=${bamNameList[$SLURM_ARRAY_TASK_ID]}_1.fastq.gz \\\n")
                fh.writelines("F2=${bamNameList[$SLURM_ARRAY_TASK_ID]}_2.fastq.gz \\\n")
		fh.writelines("collate=1 \\\n")
		fh.writelines("exclude=QCFAIL,SECONDARY,SUPPLEMENTARY \\\n")
		fh.writelines("filename=" + bamDir + "/${bamNameList[$SLURM_ARRAY_TASK_ID]}.bam \\\n")
		fh.writelines("outputdir=" + workDir + "/" + outName + "/TEMP/${bamNameList[$SLURM_ARRAY_TASK_ID]}/fastq \\\n")
